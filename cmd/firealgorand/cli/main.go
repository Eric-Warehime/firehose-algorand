...
var rootLog, _ = logging.RootLogger("firealgorand", "github.com/Eric-Warehime/firehose-algorand/cmd/firealgorand/cli")

var RootCmd = &cobra.Command{Use: "firealgorand", Short: "Algorand on StreamingFast"}
var allFlags = make(map[string]bool) // used as global because of async access to cobra init functions

func Main() {
	cobra.OnInitialize(func() {
		allFlags = flags.AutoBind(RootCmd, "firealgorand")
	})
	...
	derr.Check("algorand-blockchain", RootCmd.Execute())
}

var startCmdExample = `firealgorand start reader-node`
